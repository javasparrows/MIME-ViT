    ...
    with open(f'{root_path}/logs/training_log.csv', 'w', newline='') as f:
        ...
        for epoch in range(epochs):  # エポック数
            ...
            for images, true_masks in train_loader:
                ...
                seg_loss = dice_loss(torch.sigmoid(masks_pred), true_masks.float(), multiclass=multiclass, weight_for_empty_mask=2.0)
            ...
            with torch.no_grad():
                for images, true_masks in val_loader:
                    ...
                    correct += (predicted == true_masks).sum().item()
            ...
            save_iou_score(epoch, mean_iou_score, mean_dice_score, root_path)
            
            print(f"Mean IoU score: {mean_iou_score:.4f}, Mean Dice score: {mean_dice_score:.4f}")
...
def save_iou_score(epoch, mean_iou_score, mean_dice_score, root_path):
    with open(f'{root_path}/logs/metrics_log.csv', 'a', newline='') as f:
        writer = csv.writer(f)
        if f.tell() == 0:
            writer.writerow(["epoch", "mean_iou", "mean_dice"])
        writer.writerow([epoch, round(mean_iou_score, 4), round(mean_dice_score, 4)])

def dice_score(masks_pred, true_masks):
    masks_pred_bin = torch.where(torch.sigmoid(masks_pred.squeeze()) > 0.5, 1, 0)
...
def test(model, test_dataset, test_filenames_hasmask_true, device, verbose=False, leave=True):
    model.eval()
    ...
    with torch.no_grad():
        for idx, filename in enumerate(tqdm(test_filenames_hasmask_true, leave=leave)):
            image, true_masks, category_ids = get_image_and_masks(test_dataset, filename)
            image = image.to(device)
            
            for true_mask, category_id in zip(true_masks, category_ids):
                ...
                test_results['category_ids'].append(category_id)
...
def test_with_nomask(model, test_dataset, test_filenames_hasmask_true, test_filenames_hasmask_false, device, verbose=False, leave=True):
    model.eval()
    ...
    with torch.no_grad():
        for mask_status, filenames in [("hasmask_true", test_filenames_hasmask_true), ("hasmask_false", test_filenames_hasmask_false)]:
        # for mask_status, filenames in [("hasmask_false", test_filenames_hasmask_false), ("hasmask_true", test_filenames_hasmask_true)]:
            for idx, filename in enumerate(tqdm(filenames, leave=leave)):
                image, true_masks, category_ids = get_image_and_masks(test_dataset, filename)
                image = image.to(device)
                masks_pred = model(image)
                ...
                for true_mask, category_id in zip(true_masks, category_ids):
                    ...
                    if mask_status == "hasmask_true":
                        ...
                        predicted_mask_area = torch.sum(masks_pred > 0.5).item()
                    ...
                    test_results[mask_status]['filenames'].append(filename)
                    ...
                    test_results[mask_status]['category_ids'].append(category_id)

    return test_results


def compute_ensemble_scores(true_masks, ensemble_masks_preds_mean, mask_status):
    ensemble_ious = []
    ensemble_dices = []
    ...
    for idx, masks_pred in enumerate(ensemble_masks_preds_mean):
        ...
        if mask_status == "hasmask_true":
            iou = iou_score(masks_pred.unsqueeze(0), true_mask_tensor.unsqueeze(0))
            dice = dice_score(masks_pred.unsqueeze(0), true_mask_tensor.unsqueeze(0))
        else:
            # Proposed accuracy computation for hasmask=False
            predicted_mask_area = torch.sum(masks_pred > 0.5).item()
            if predicted_mask_area == 0:
...
def iou_score(masks_pred, true_masks):
    masks_pred = torch.where(torch.sigmoid(masks_pred.squeeze()) > 0.5, 1, 0)
...
def save_prediction_images(filename, image, true_masks, masks_pred, category_id, save_dir, dice, verbose):
    ...
    # Save the figure
    figure_save_path = os.path.join(save_dir, f"{filename}_dice={dice:.4f}_id={category_id}.png")
...
def dice_coeff(input: Tensor, target: Tensor, reduce_batch_first: bool = False, epsilon: float = 1e-6):
    ...
    assert input.dim() == 4 or not reduce_batch_first  # now we accept 4D tensor

    sum_dim = (-2, -1) if input.dim() == 3 or not reduce_batch_first else (-1, -2, -3)
...
def multiclass_dice_coeff(input: Tensor, target: Tensor, reduce_batch_first: bool = False, epsilon: float = 1e-6):
    ...
    return dice_coeff(input, target, reduce_batch_first, epsilon)  # no need to flatten


def dice_loss(input: Tensor, target: Tensor, multiclass: bool = True, weight_for_empty_mask: float = 2.0):
    # Dice loss (objective to minimize) between 0 and 1
    fn = multiclass_dice_coeff if multiclass else dice_coeff
    base_loss = 1 - fn(input, target, reduce_batch_first=True)
    
    # Check if target mask is empty and input predicts a mask
    no_true_mask = torch.sum(target) == 0
    mask_predicted = torch.sum(input) > 0
    
    # Apply weight if the situation matches
    if no_true_mask and mask_predicted:
        return weight_for_empty_mask * base_loss
    else:
        return base_loss
